...
class rank_select<bit::vector<uint64_t>, 64, 8>
{
    ...
    private:
        ...
        void build_index();
        inline std::size_t sub_block_rank(uint64_t block) const { return interleaved_blocks[block * 2]; }
};

rank_select<bit::vector<uint64_t>, 64, 8>::rank_select(bit::vector<uint64_t>&& vector)
    : _data(vector), payload(vector.vector_data())// Possible undefined behaviour
{}
